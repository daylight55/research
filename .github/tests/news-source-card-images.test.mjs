...
test('published news articles provide images for every NewsSourceCard', () => {
	...
	for (const file of files) {
		...
		cards.forEach((card, index) => {
			assert.ok(hasProp(card, 'imageUrl'), `${file} NewsSourceCard ${index + 1} should include imageUrl`)
			assert.ok(hasProp(card, 'imageAlt'), `${file} NewsSourceCard ${index + 1} should include imageAlt`)
			assert.ok(
