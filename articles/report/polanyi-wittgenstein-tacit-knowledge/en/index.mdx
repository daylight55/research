...
```mermaid
flowchart LR
  P["Polanyi<br/>positive theory of tacit knowing"] --> C["Common ground<br/>Resisting reductionism<br/>Emphasizing background conditions"]
  W["Wittgenstein<br/>limits of language"] --> C
  C --> D["Differences<br/>Epistemology and logical therapy"]
```
...
flowchart TB
  ...
  SAYSHOW["Said / shown"] --> LIM["limits of language"]
  LIM --> PRACTICE["Practice / background"]
```
...
## 8. Lessons for Knowledge Transfer

This argument matters for knowledge transfer because it provides judgments that are not "over-summarized" when extracting tacit knowledge and transferring knowledge.
...
5. **Verify in practice**: Documented rules have meaning only when they are used in the field.
   ...
   Source note: The knowledge-transfer lessons are interpretations from Polanyi's [The Tacit Dimension](https://press.uchicago.edu/ucp/books/book/chicago/T/bo6035368.html) and Tractatus [Wikisource](https://en.wikisource.org/wiki/Tractatus_Logico-Philosophicus). "Do not over-summarize" here is not a single word in the original text, but a secondary design lesson.
