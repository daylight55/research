...
assert.match(
	readFileSync('ops/codex/prompts/daily-trend-news.md', 'utf8'),
	/mix-alignment\.json[\s\S]*?MIX/,
	'Daily Trend News prompt should require a MIX alignment file when localized news is generated'
)
...
assert.match(
	workflow,
	/- name: Build generated site[\s\S]*?id: generated_site_build[\s\S]*?continue-on-error: true[\s\S]*?run: pnpm build[\s\S]*?- name: Report skipped generated site build[\s\S]*?steps\.generated_site_build\.outcome == 'failure'/,
	'Daily Issue Research should treat generated site build failures as skipped scheduled publication'
)

assert.match(
	workflow,
	/- name: Create generated article cache archive[\s\S]*?steps\.generated_site_build\.outcome == 'success'[\s\S]*?- name: Save generated article cache[\s\S]*?steps\.generated_site_build\.outcome == 'success'[\s\S]*?- name: Create research pull request[\s\S]*?steps\.generated_site_build\.outcome == 'success'/,
	'Daily Issue Research should only cache and publish generated reports after a successful site build'
)
...
assert.match(
	...
	'Daily Issue Research run repository tests and MIX alignment before generated report publication'
)
...
assert.match(
	testWorkflow,
	/- name: Validate all mixed article alignment[\s\S]*?run: node ops\/scripts\/validate-mix-alignment\.mjs(?! --changed)/,
	'All test runs should validate every mixed article alignment, including articles generated by Actions'
)

assert.doesNotMatch(
	testWorkflow,
	/- name: Validate all mixed article alignment[\s\S]*?if: github\.event_name != 'pull_request'/,
	'Pull request tests should not skip full mixed article alignment validation'
)
...
assert.doesNotMatch(
	workflow,
	/pnpm test:hero-images|hero_image_tests|daily-research-hero-images\.log/,
	'Daily Issue Research should not block generated report publication on hero image tests'
)
...
assert.match(
	trendWorkflow,
	/- name: Verify generated mixed news alignment[\s\S]*?node ops\/scripts\/validate-mix-alignment\.mjs --changed/,
	'Daily Trend News should reject generated bilingual news without mix-alignment before opening a PR'
)
...
assert.doesNotMatch(
	testWorkflow,
	/- name: Validate all mixed article alignment[\s\S]*?if: github\.event_name != 'pull_request'/,
	'Test workflow should run full mixed article alignment validation on pull requests as well as main pushes'
)
