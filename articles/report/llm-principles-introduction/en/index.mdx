...
The center of Transformer is self-attention. A simple way to think about it is that each token decides which other tokens in the sentence it should pay attention to, and how much. Pronouns, conditions, and references often need to connect to words that are far away, not just nearby. Transformer computes those links through attention weights rather than through sequential recurrence. <SourceNote>[Attention Is All You Need](https://arxiv.org/abs/1706.03762) showed that sequence transformation can be done with attention alone, without recurrence or convolution.</SourceNote>
