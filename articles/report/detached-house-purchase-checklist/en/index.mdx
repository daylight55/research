...
description: 'We will organize the total cost, land, building, contract, surrounding environment, and maintenance costs that you should check from a deployment perspective when purchasing a detached house in Japan.'
...
```mermaid
flowchart TD
  A["Find a candidate property"] --> B["Build a full budget<br/>Price + fees + reserve + repairs"]
  B --> C{"Is the land safe?"}
  C -->|Unchecked| C1["Check registry, access, boundaries, private roads, zoning, hazards"]
  C -->|OK| D{"Is the building sound?"}
  D -->|Unchecked| D1["Check inspection, repair history, warranty, seismic resistance, insulation"]
  D -->|OK| E{"Can the contract be exited?"}
  E -->|Unchecked| E1["Check loan contingency, deposit cancellation, nonconformity liability, handover terms"]
  E -->|OK| F["Purchase decision"]
  C1 --> C
  D1 --> D
  E1 --> E
```
...
```mermaid
flowchart LR
  A["Registry"] --> A1["Owner<br/>Mortgage<br/>Easement<br/>Leasehold"]
  B["Roads"] --> B1["Road access<br/>Private-road burden<br/>Setback<br/>Passage and excavation consent"]
  C["Zoning"] --> C1["Use district<br/>Building coverage<br/>FAR<br/>Urbanization-control area"]
  D["Boundaries"] --> D1["Boundary markers<br/>Survey map<br/>Encroachment<br/>Retaining wall"]
  E["Hazards"] --> E1["Flood<br/>Landslide<br/>Tsunami<br/>Storm surge<br/>Ground"]
  A1 --> F["Rebuildability, use, collateral value"]
  B1 --> F
  C1 --> F
  D1 --> F
  E1 --> F
