...
```mermaid
flowchart TD
  A["実践の中の出来事<br/>判断・違和感・手がかり"] --> B["観察<br/>何を見ているかを記録"]
  B --> C["語り<br/>本人の言葉で説明"]
  C --> D["対話<br/>なぜそう見えたかを掘る"]
  D --> E["仮の形式化<br/>手順・原則・パターン"]
  E --> F["現場での再検証<br/>例外・失敗・境界条件"]
  F --> G["実務知として運用<br/>教育・設計・AI支援"]
  F --> B
```
...
```mermaid
flowchart LR
  S["Socialization<br/>暗黙知から暗黙知<br/>同行・模倣・共同経験"] --> E["Externalization<br/>暗黙知から形式知<br/>比喩・対話・モデル化"]
  E --> C["Combination<br/>形式知から形式知<br/>文書・データ・体系化"]
  C --> I["Internalization<br/>形式知から暗黙知<br/>実践・訓練・身体化"]
  I --> S
```
...
flowchart TD
  ...
  O --> A["手がかり抽出"]
  I --> A
  A --> K["知識資産化<br/>ケース・手順・動画・レビュー観点"]
  K --> AI["AI支援<br/>検索・対話・訓練・振り返り"]
  AI --> V["現場検証"]
  V --> A
```
